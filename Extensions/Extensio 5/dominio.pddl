(define (domain menu-setmanal)
  (:requirements :strips :typing :equality :fluents)    ;; afegim fluents 
  ...
  (:action assignar-segon
    ...
    :precondition (and
      ...
      (tipus-es ?s ?t) 
      (not (= ?t0 ?t))
      ;; Comprobem que estiguem en el rang de valors de calories [calories-min, calories-max]
      (>=   (+ (calories ?p) (calories ?s))   (calories-min)  )
      (<=   (+ (calories ?p) (calories ?s))   (calories-max)  )
    )
    :effect (and
      (segon-assignat ?d)
      (segon-es ?d ?s)
